...
Cost figures come exclusively from cost_service.resolve() — the single
source of truth shared by all pages.
"""
...
def _namespace_waste(pods_payload: dict) -> list[dict]:
    ...
    for ns in ns_cpu:
        ...
        c = (ns_cpu[ns] * CPU_COST_PER_CORE_HOUR + ns_mem[ns] * MEMORY_COST_PER_GB_HOUR) * HOURS_PER_MONTH
        avg_cpu   = ns_cpu[ns] / max(ns_pods[ns], 1)
        waste_pct = min(max(0.0, (0.1 - avg_cpu) / 0.1 * 100) if avg_cpu < 0.1 else 0.0, 95.0)
        results.append({
            'namespace': ns,
            'monthly_cost': round(c, 2),
...
async def get_executive_overview(
    ...
    """
    Executive overview — cost figures from cost_service.resolve(), infra
    counts from agent metrics. No dummy data.
    """
    try:
        ...
        for cluster_data in agent_clusters:
            ...
            snapshot = await cost_service.resolve(cname)
            cost     = snapshot.total_monthly_cost
            savings  = snapshot.savings_potential
            ...
            # ── Namespace waste (for top-waste list only) ─────────────────────
