...
const ISO27001ComplianceInner: React.FC = () => {
  ...
  const handleFix = async (control: FailedControl) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/iso27001/fix/${encodeURIComponent(control.control_id)}${clusterParam}`, {
        method: 'POST',
      });
  ...
  const handleSaveException = async () => {
    ...
    setActionMessage(null);
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/iso27001/exception${clusterParam}`, {
        method: 'POST',
    ...
    <Box p={3} sx={{ bgcolor: colors.background, minHeight: '100vh', color: colors.textPrimary }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3, mt: 2 }}>
        {[
          ...
          { label: 'Status', value: data.certification_status, color: data.certification_status === 'Certified' ? colors.success : colors.warning },
          ...
          { label: 'Certification Date', value: new Date(data.certification_date).toLocaleDateString(), color: colors.textPrimary },
          { label: 'Next Audit', value: new Date(data.next_audit).toLocaleDateString(), color: colors.warning },
        ].map((k) => (
      ...
      <Card sx={{ bgcolor: colors.surface, border: `1px solid ${colors.border}`, mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="caption" sx={{ color: colors.textSecondary, display: 'block', mb: 2 }}>
            Each domain score is computed from real workload security signals collected from the cluster.
          </Typography>
          <TableContainer>
            <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {(data.domains || []).map((d) => (
                  <TableRow key={d.domain} hover sx={{ '&:hover': { bgcolor: colors.surfaceHover } }}>
                    <TableCell sx={{ color: colors.textPrimary, fontWeight: 600, fontSize: 13, borderColor: colors.border }}>{d.domain}</TableCell>
                    <TableCell align="right" sx={{ color: colors.textSecondary, fontSize: 13, borderColor: colors.border }}>{d.controls}</TableCell>
                    <TableCell align="right" sx={{ color: colors.success, fontWeight: 600, fontSize: 13, borderColor: colors.border }}>{d.passed}</TableCell>
                    <TableCell align="right" sx={{ color: d.failed > 0 ? colors.danger : colors.textSecondary, fontWeight: 600, fontSize: 13, borderColor: colors.border }}>{d.failed}</TableCell>
                    <TableCell sx={{ borderColor: colors.border }}>
                      <Box display="flex" alignItems="center" gap={1}>
                        <LinearProgress
                          variant="determinate"
                          value={d.score}
                          sx={{ flex: 1, height: 6, borderRadius: 3, bgcolor: colors.border, '& .MuiLinearProgress-bar': { bgcolor: scoreColor(d.score) } }}
                        />
                        <Typography variant="caption" fontWeight={700} sx={{ color: scoreColor(d.score), fontSize: 12 }}>{d.score}%</Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                ))}
                  ...
                  {failedDetail.map((c) => {
                    ...
                    return (
                      <TableRow key={c.control_id} hover sx={{ '&:hover': { bgcolor: colors.surfaceHover } }}>
                        ...
                        <TableCell sx={{ color: colors.textSecondary, fontSize: 11, borderColor: colors.border, whiteSpace: 'nowrap' }}>{c.domain}</TableCell>
                        <TableCell sx={{ borderColor: colors.border }}>
      ...
      >
        ...
        <DialogContent sx={{ pt: 2, display: 'grid', gap: 2 }}>
          <Typography variant="body2" sx={{ color: colors.textSecondary }}>
            Use this only when you intentionally want to keep the current ISO 27001 deviation and record why no change should be made now.
          </Typography>
          <TextField
            label="Control"
