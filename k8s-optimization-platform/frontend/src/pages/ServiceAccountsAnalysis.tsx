...
const ServiceAccountsAnalysis: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/v1/security/rbac-analysis/service-accounts${clusterParam}`)
      .then(res => res.json())
  ...
  return (
    <Box sx={{ flexGrow: 1, p: 3 }}>
      <Typography variant="h4" gutterBottom sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        <SecurityIcon /> Service Accounts
      </Typography>
      <Grid container spacing={3} sx={{ mb: 3 }}>
        <Grid item xs={12} md={3}>
          <Card><CardContent>
            <Typography color="textSecondary" gutterBottom>Service Account Score</Typography>
            <Typography variant="h3">{data.service_account_score}</Typography>
          </CardContent></Card>
        </Grid>
        <Grid item xs={12} md={3}>
          <Card><CardContent>
            <Typography color="textSecondary" gutterBottom>Total Service Accounts</Typography>
            <Typography variant="h3">{data.total_service_accounts}</Typography>
          </CardContent></Card>
        </Grid>
        <Grid item xs={12} md={3}>
          <Card><CardContent>
            <Typography color="textSecondary" gutterBottom>Unused</Typography>
            <Typography variant="h3">{data.unused}</Typography>
          </CardContent></Card>
        </Grid>
        <Grid item xs={12} md={3}>
          <Card><CardContent>
            <Typography color="textSecondary" gutterBottom>Service Accounts</Typography>
            <Typography variant="h3">{data.service_accounts}</Typography>
          </CardContent></Card>
        </Grid>
      </Grid>
      <Card>
        <CardContent>
          ...
          <TableContainer component={Paper} sx={{ mt: 2 }}>
            <Table>
              ...
              <TableBody>
                {data.service_accounts && data.service_accounts.slice(0, 50).map((item: any, i: number) => (
                  <TableRow key={i}>
