...
const MemoryAnalysis: React.FC = () => {
  ...
  const [pods, setPods] = useState<MemoryAnalysisItem[]>([]);
  ...
  const fetchData = async (clusterId: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/v1/pods/memory-analysis${param}`);
      const data = await response.json();
      setPods(data);
    } catch (err) {
      setError('Failed to fetch memory analysis data');
      console.error('Error fetching memory analysis:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'oom_risk': return <ErrorIcon fontSize="small" />;
      default: return null;
    }
  };
  ...
  const oomRisk = filteredPods.filter(p => p.status === 'oom_risk').length;
  ...
  const avgUtilization = totalPods > 0 
    ? filteredPods.reduce((sum, p) => sum + p.memory_utilization, 0) / totalPods 
    : 0;
  const totalOOMKills = filteredPods.reduce((sum, p) => sum + p.oom_kills, 0);
    ...
    <Box>
      ...
      <Grid container spacing={3} mb={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              <Typography color="textSecondary" gutterBottom>OOM Risk</Typography>
              <Typography variant="h4" color="error.main">{oomRisk}</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              <Typography color="textSecondary" gutterBottom>Total OOM Kills</Typography>
              <Typography variant="h4" color="error.main">{totalOOMKills}</Typography>
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              <Typography color="textSecondary" gutterBottom>Avg Memory Utilization</Typography>
          ...
          <TableBody>
            {filteredPods.map((pod, index) => (
              <TableRow key={index} hover>
                ...
                <TableCell>{formatMemory(pod.memory_current)}</TableCell>
                <TableCell>{formatMemory(pod.memory_peak)}</TableCell>
                <TableCell>{formatMemory(pod.memory_request)}</TableCell>
                <TableCell>{formatMemory(pod.memory_limit)}</TableCell>
                <TableCell>
                  <Box display="flex" alignItems="center" gap={1}>
                    <LinearProgress
                      variant="determinate"
                      value={Math.min(pod.memory_utilization, 100)}
                      sx={{ width: 60, height: 8, borderRadius: 1 }}
                      color={pod.memory_utilization > 85 ? 'error' : pod.memory_utilization > 70 ? 'warning' : 'success'}
                    />
                    <Typography variant="body2">{pod.memory_utilization.toFixed(1)}%</Typography>
                  </Box>
                </TableCell>
                <TableCell>
                  <Typography 
                    variant="body2" 
                    color={pod.memory_waste_percent > 50 ? 'warning.main' : 'text.secondary'}
                  >
                    {pod.memory_waste_percent.toFixed(1)}%
                  </Typography>
                </TableCell>
                <TableCell>
                  {pod.oom_kills > 0 ? (
