...
const HIPAAComplianceInner: React.FC = () => {
  ...
  const handleFix = async (control: FailedControl) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/hipaa/fix/${encodeURIComponent(control.control_id)}${clusterParam}`, {
        method: 'POST',
      });
  ...
  const handleSaveException = async () => {
    ...
    setActionMessage(null);
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/hipaa/exception${clusterParam}`, {
        method: 'POST',
    ...
    <Box p={3} sx={{ bgcolor: colors.background, minHeight: '100vh', color: colors.textPrimary }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3, mt: 2 }}>
        {[
          ...
          { label: 'PHI Protected', value: data.phi_protected ? 'Yes' : 'No', color: data.phi_protected ? colors.success : colors.danger },
          { label: 'Last Risk Assessment', value: new Date(data.last_risk_assessment).toLocaleDateString(), color: colors.textPrimary },
        ].map((k) => (
      ...
      <Card sx={{ bgcolor: colors.surface, border: `1px solid ${colors.border}`, mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="caption" sx={{ color: colors.textSecondary, display: 'block', mb: 2 }}>
            Each safeguard score is computed from real workload security signals collected from the cluster.
          </Typography>
          <TableContainer>
            <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {(data.safeguards || []).map((s) => (
                  <TableRow key={s.safeguard} hover sx={{ '&:hover': { bgcolor: colors.surfaceHover } }}>
                    <TableCell sx={{ color: colors.textPrimary, fontWeight: 600, fontSize: 13, borderColor: colors.border }}>{s.safeguard}</TableCell>
                  ...
                  {failedDetail.map((c) => {
                    ...
                    return (
                      <TableRow key={c.control_id} hover sx={{ '&:hover': { bgcolor: colors.surfaceHover } }}>
                        ...
                        <TableCell sx={{ color: colors.textSecondary, fontSize: 11, borderColor: colors.border, whiteSpace: 'nowrap' }}>{c.safeguard}</TableCell>
                        <TableCell sx={{ borderColor: colors.border }}>
      ...
      >
        ...
        <DialogContent sx={{ pt: 2, display: 'grid', gap: 2 }}>
          <Typography variant="body2" sx={{ color: colors.textSecondary }}>
            Use this only when you intentionally want to keep the current HIPAA deviation and record why no change should be made now.
          </Typography>
          <TextField
            label="Control"
