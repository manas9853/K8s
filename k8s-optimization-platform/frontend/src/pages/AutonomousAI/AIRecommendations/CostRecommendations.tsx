...
const KpiCard: React.FC<{ label: string; value: string | number; accent?: string; icon?: React.ReactNode }> = ({ label, value, accent, icon }) => (
  <Box sx={{ bgcolor: DK.surface, border: `1px solid ${DK.border}`, borderRadius: 2, p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
    {icon && <Box sx={{ color: accent ?? colors.info }}>{icon}</Box>}
    <Box>
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, py: 1, borderBottom: `1px solid ${DK.border}` }}>
      ...
      <Box sx={{ flex: 1, bgcolor: DK.surface2, borderRadius: 1, height: 6, overflow: 'hidden' }}>
        <Box sx={{ width: `${pct}%`, height: '100%', bgcolor: pct > 70 ? colors.danger : pct > 40 ? colors.warning : colors.info, borderRadius: 1, transition: 'width 0.5s' }} />
      </Box>
...
const RecCard: React.FC<{
  ...
  return (
    <Box sx={{
      ...
      '&:hover': { borderColor: done ? DK.border : `${colors.info}66`, borderLeftColor: color },
    }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 0.75, flexShrink: 0 }}>
        <Typography sx={{ color: colors.success, fontWeight: 700, fontSize: '0.95rem' }}>${rec.savings}/mo</Typography>
        {done ? (
      ...
      {!loading && data && (
        <>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 2, mb: 3 }}>
            ...
            <KpiCard label="Potential Savings" value={`$${data.potential_savings}/mo`} accent={colors.success} icon={<TrendingDownIcon />} />
            {selectedCount > 0 && (
              <KpiCard label="Selected Savings" value={`$${selectedTotal.toFixed(2)}/mo`} accent={colors.info} icon={<AttachMoneyIcon />} />
            )}
          </Box>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
            ...
            {selectedCount > 0 && (
              <Button variant="contained" size="small"
                ...
                sx={{ bgcolor: colors.info, '&:hover': { bgcolor: colors.info }, textTransform: 'none', fontWeight: 600 }}>
