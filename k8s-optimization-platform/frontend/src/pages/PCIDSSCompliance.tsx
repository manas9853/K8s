...
const PCIDSSComplianceInner: React.FC = () => {
  ...
  const handleFix = async (control: FailedControl) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/pci-dss/fix/${control.control_id}${clusterParam}`, {
        method: 'POST',
      });
  ...
  const handleSaveException = async () => {
    ...
    setActionMessage(null);
    try {
      const response = await fetch(`${API_BASE_URL}/v1/compliance/pci-dss/exception${clusterParam}`, {
        method: 'POST',
    ...
    <Box p={3} sx={{ bgcolor: colors.background, minHeight: '100vh', color: colors.textPrimary }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3, mt: 2 }}>
        {[
          ...
          { label: 'Status', value: data.compliance_status, color: data.compliance_status === 'Compliant' ? colors.success : colors.danger },
          ...
          { label: 'Next Assessment', value: new Date(data.next_assessment).toLocaleDateString(), color: colors.warning },
        ].map((k) => (
      ...
      <Card sx={{ bgcolor: colors.surface, border: `1px solid ${colors.border}`, mb: 3 }}>
        <CardContent>
          ...
          <Typography variant="caption" sx={{ color: colors.textSecondary, display: 'block', mb: 2 }}>
            Each requirement score is computed from real workload security signals collected from the cluster.
          </Typography>
          <TableContainer>
            <Table size="small">
              <TableHead>
              ...
              <TableBody>
                {(data.requirements || []).map((r) => (
                  <TableRow key={r.req} hover sx={{ '&:hover': { bgcolor: colors.surfaceHover } }}>
                    <TableCell sx={{ color: colors.textPrimary, fontWeight: 600, fontSize: 13, borderColor: colors.border }}>{r.req}</TableCell>
      ...
      >
        ...
        <DialogContent sx={{ pt: 2, display: 'grid', gap: 2 }}>
          <Typography variant="body2" sx={{ color: colors.textSecondary }}>
            Use this only when you intentionally want to keep the current PCI DSS deviation and record why no change should be made now.
          </Typography>
          <TextField
            label="Control"
